...
Its default-on presentation projection may move one clean new task into its own space grouped under its project without changing endpoint authority or lifecycle ownership; [Presentation spaces](herdr-backend.md#presentation-spaces) owns that conditional design, the Herdr version floor its unconfigured default is gated behind, and its narrow home-local restored-shell cleanup at locked session start.
...
A `https://<region>.console.aws.amazon.com/codesuite/codecommit/repositories/<repo>/pull-requests/<n>?region=<region>` URL (see [docs/verification/codecommit-merge-watch.md](verification/codecommit-merge-watch.md)) invokes the AWS CLI's own merge API, defaults to squash to match the GitHub default, maps an explicit `--merge` or `--fast-forward` onto CodeCommit's own three-way and fast-forward APIs, refuses `--rebase` because CodeCommit has no rebase merge rather than substituting a method the caller did not ask for, and takes no other extra argument because the CLI would read one as its own option.
The console spells that one pull request several ways, so the `/details` segment its address bar carries and a missing `?region=` query are both accepted and normalised to the single form above before anything is recorded; a query naming a region the host does not is still refused rather than resolved to the host's.
Normalising rather than merely accepting is what makes those spellings safe, because `bin/fm-pr-poll.sh` rebuilds the URL from the stored parts and a watch armed on an unnormalised URL would never match and so never report the merge.
That path merges only after one live read confirms the pull request is open, its approval rules evaluate as approved, and the merge method it will actually use reports no conflicts, and it binds the merge to the verified head through `--source-commit-id`.
Its AWS profile comes from the task worktree's own git remote, the only place that profile is recorded, and an underivable one refuses rather than falling back to an unauthenticated default credential chain.
CodeCommit closes a pull request whether it was merged or abandoned, so only `mergeMetadata.isMerged` is read as a landed merge and every other outcome refuses.
