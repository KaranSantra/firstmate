...
# while the away-posture record exists a green merge proceeds under away authority
# and the ledger tags it away (the per-task grant is retired), and the accepted
# authority is persisted so a poll that later observes the merge can attribute it.
# --allow-red is parsed before the CodeCommit extra-argument refusal, so it is
# refused by name.
test_codecommit_away_authority_and_allow_red() {
  local case_dir rc record

  case_dir=$(make_codecommit_case codecommit-away-green)
  mkdir -p "$case_dir/home"
  write_away_record "$case_dir" --words 'merge task-x1 when green'
  FM_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 "$CC_URL" \
    > "$case_dir/stdout" 2> "$case_dir/stderr" \
    || fail "codecommit-away-green: a green CodeCommit merge should land under away authority: $(cat "$case_dir/stderr")"
  [ -n "$(aws_merge_line "$case_dir/aws.log")" ] \
    || fail "codecommit-away-green: the away merge never reached CodeCommit"
  assert_grep "merge landed: task-x1 $CC_URL away" "$case_dir/state/.wake-queue" \
    "codecommit-away-green: the durable outcome did not tag away"
  assert_no_grep 'away-grant' "$case_dir/state/.wake-queue" \
    "codecommit-away-green: the retired away-grant tag reappeared"
  record="$case_dir/state/task-x1.merge-authority"
  [ -f "$record" ] || fail "codecommit-away-green: the accepted merge authority was not persisted"
  [ "$(sed -n 2p "$record")" = codecommit ] && [ "$(sed -n 6p "$record")" = away ] \
    || fail "codecommit-away-green: the persisted authority does not name codecommit and away"
